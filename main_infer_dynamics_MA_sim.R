...
nchains <- 3
n_temperatures <- 10
mcmcPars_ct_pt <- c("iterations"=50000,"popt"=0.44,"opt_freq"=2000,
                    "thin"=50,"adaptive_period"=20000,"save_block"=100)
...
ct_dat <- read_csv("~/Documents/GitHub/variant_viral_loads/data/simulated_indiv_data.csv")
...
virosolver_pars[virosolver_pars$names %in% c("nu","rho"), "values"] <- c(2,0.05)
...
res <- foreach(j=1:nchains,.packages = c("extraDistr","tidyverse","patchwork","virosolver","lazymcmc")) %dopar% {
  ...
  output <- run_MCMC(parTab=startTab,
                     ...
                     filename=paste0("chains/",runname,"/",runname,"_",j),
...
p_incidence1 + geom_line(data=sir_scaled,aes(x=t+5,y=inc,col=variant)) + scale_x_continuous(breaks=seq(0,100,by=5))# + coord_cartesian(xlim=c(35,135))
