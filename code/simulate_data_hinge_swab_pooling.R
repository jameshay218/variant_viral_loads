#################################################################
## SCRIPT 3: SIMULATE MANY SWAB VIRAL LOAD TRAJECTORIES WITH AND WITHOUT OBSERVATION ERROR
#################################################################
...
obs_vl_melted <- obs_vl_melted %>% left_join(sim_pars %>% dplyr::select(i, incu_period))
...
p_obs_traj <- ggplot(obs_vl_melted %>% mutate(t = t-inf_time) %>% filter(t >0 & t < 50)%>%
  ...
  geom_vline(aes(xintercept=incu_period),col="red",linetype="dashed") +
  theme_bw() +
